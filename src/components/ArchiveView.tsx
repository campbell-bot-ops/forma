...
import { TrendingUp, BarChart2, ShieldAlert, Calendar, Layout, ChevronDown, ChevronUp, Share2 } from 'lucide-react';
...
export default function ArchiveView({ workoutHistory, onShareWorkout }: ArchiveViewProps) {
  ...
  if (!isHistoryEmpty) {
    ...
    if (previousSessions.length > 0) {
      ...
      const tonnagePctDiff = ((recentSession.actualTonnage - avgPrevTonnage) / avgPrevTonnage) * 100;
    ...
    // Volume Consistency calculation: ratio of completed sets vs planned sets
    // (Every exercise has standard sets, e.g. 4 sets. We count logged sets vs total baseline sets)
    let totalLoggedSets = 0;
    let totalTargetSets = 15; // default estimation
    workoutHistory.forEach(log => {
      Object.keys(log.logs).forEach(exId => {
        totalLoggedSets += log.logs[exId].length;
      });
    });
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="w-full">
        <div className="glass-panel border-amber-950/20 bg-amber-950/5 rounded-2xl p-5 flex items-start gap-4">
      ...
      {/* Dynamic Interactive Multi-Axis Chart */}
      <motion.div variants={itemVariants} className="glass-panel rounded-2xl p-5 flex flex-col gap-4 relative">
      ...
      {/* Timeline of Completed Workout Logs (Only visible when user has actually completed workouts) */}
      {!isHistoryEmpty && (
        <motion.div variants={itemVariants} className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-3">
            {workoutHistory.map((historyItem: any, idx: number) => {
              ...
              return (
                <div key={uniqueId} className="glass-panel rounded-xl p-4 flex flex-col gap-2">
                  <div 
